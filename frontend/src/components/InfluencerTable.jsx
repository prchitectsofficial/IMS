...
function InfluencerTable({
  ...
  // Pagination helpers
  function getPageNums(currentPage, maxPage) {
  ...
  const pgBtn = (disabled) =>
    `px-2 py-1 border rounded text-sm ${disabled ? 'opacity-40 cursor-default' : 'hover:bg-gray-100 cursor-pointer'}`;
  const pgBtnActive = 'px-2 py-1 border rounded text-sm bg-blue-600 text-white border-blue-600 font-bold cursor-pointer';
          ...
          {Math.ceil(totalCount / 20) > 1 && (
            <div className="px-4 py-3 border-t border-gray-200">
              ...
              <div className="flex items-center justify-center gap-1">
                <button onClick={() => onPageChange(1)} disabled={page === 1} className={pgBtn(page === 1)}>«</button>
                <button onClick={() => onPageChange(page - 1)} disabled={page === 1} className={pgBtn(page === 1)}>‹</button>
                {getPageNums(page, Math.ceil(totalCount / 20)).map(n => (
                  <button key={n} onClick={() => onPageChange(n)}
                    className={n === page ? pgBtnActive : pgBtn(false)}>{n}</button>
                ))}
                <button onClick={() => onPageChange(page + 1)} disabled={page >= Math.ceil(totalCount / 20)} className={pgBtn(page >= Math.ceil(totalCount / 20))}>›</button>
                <button onClick={() => onPageChange(Math.ceil(totalCount / 20))} disabled={page >= Math.ceil(totalCount / 20)} className={pgBtn(page >= Math.ceil(totalCount / 20))}>»</button>
              </div>
            </div>
          )}
        </div>
        ...
        {totalPages > 1 && (
          <div className="px-4 py-3 border-t border-gray-200">
            ...
            <div className="flex items-center justify-center gap-1">
              <button onClick={() => onPageChange(1)} disabled={page === 1} className={pgBtn(page === 1)}>«</button>
              <button onClick={() => onPageChange(page - 1)} disabled={page === 1} className={pgBtn(page === 1)}>‹</button>
              {getPageNums(page, totalPages).map(n => (
                <button key={n} onClick={() => onPageChange(n)}
                  className={n === page ? pgBtnActive : pgBtn(false)}>{n}</button>
              ))}
              <button onClick={() => onPageChange(page + 1)} disabled={page >= totalPages} className={pgBtn(page >= totalPages)}>›</button>
              <button onClick={() => onPageChange(totalPages)} disabled={page >= totalPages} className={pgBtn(page >= totalPages)}>»</button>
            </div>
          </div>
        )}
      </div>
