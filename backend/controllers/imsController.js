...
async function enrichRows(rows) {
  ...
  // 1a. Admin notes COUNT — independent try/catch so it never gets wiped
  try {
    ...
    const countMap = new Map(countRows.map(r => [Number(r.channel_id), Number(r.cnt)]));
    rows.forEach(r => {
      const cid = r.channel_id != null ? Number(r.channel_id) : Number(r.id);
      r.admin_notes_count = countMap.get(cid) || 0;
    });
  } catch (err) {
  ...
  // 1b. Latest note text — separate try/catch so count is never affected
  try {
    const channelIds   = [...new Set(rows.map(r => r.channel_id != null ? r.channel_id : r.id))];
    const placeholders = channelIds.map(() => '?').join(',');
    const [noteRows]   = await db.execute(
      `SELECT ${tables.ADMINNOTES.CHANNEL_ID_COLUMN} as channel_id,
              ${tables.ADMINNOTES.NOTE_COLUMN} as note,
    ...
    // Keep only the latest note per channel_id
    const latestNoteMap = new Map();
    noteRows.forEach(r => {
      const cid = Number(r.channel_id);
      if (!latestNoteMap.has(cid)) latestNoteMap.set(cid, r);
    });
    rows.forEach(r => {
      const cid = r.channel_id != null ? Number(r.channel_id) : Number(r.id);
      const ln  = latestNoteMap.get(cid);
      r.latest_note = ln ? { text: ln.note || '', createdby: ln.created_by || '' } : null;
    });
  } catch (err) {
    console.warn('Latest note fetch failed:', err.message);
    rows.forEach(r => { r.latest_note = null; });
  }
...
exports.getList = async (req, res) => {
  try {
    ...
    // ES hard limit: from + size <= 10000. With size=40 (2x dedup), max page = floor(10000/40) = 250
    const pageNum  = parseInt(page) || 1;
    const limitNum = parseInt(limit) || 20;
    const esFrom   = (pageNum - 1) * limitNum;
    const ES_MAX_FROM = 9960; // 10000 - 40 (2x limit for dedup)
    const isBeyondESLimit = esFrom > ES_MAX_FROM;

    let esFailed = isPromoSort || isBeyondESLimit;
    if (isPromoSort || isBeyondESLimit) source = 'mysql';
    try {
      if (esFailed) throw new Error('skipped'); // skip ES for promo sort or deep pages
